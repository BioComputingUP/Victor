...
static const char* __rcsid__ = "@(#) $Id: matrix3.cc,v 1.5 2008-05-09 13:58:50 biocomp Exp $";
...
template <class T> vgMatrix3<T> vgMatrix3<T>::inverse() const {
    ...
    if ((newPivot = vg_abs(src.y.x)) > pivot) {
        pivot = newPivot;
        swap(src.y, src.x);
        swap(result.y, result.x);
    }
    if ((newPivot = vg_abs(src.z.x)) > pivot) {
        pivot = newPivot;
        swap(src.z, src.x);
        swap(result.z, result.x);
    }
    ...
    result.x *= pivot;
    src.x *= pivot;
    result.y -= result.x * src.y.x;
    src.y -= src.x * src.y.x;
    result.z -= result.x * src.z.x;
    src.z -= src.x * src.z.x;
    // now: first row is (1, 0, 0)
    pivot = vg_abs(src.y.y);
    if ((newPivot = vg_abs(src.z.y)) > pivot) {
        pivot = newPivot;
        swap(src.z, src.y);
        swap(result.z, result.y);
    }
    ...
    src.z *= pivot;
    // now: third row is (?, ?, 1)
    result.y -= result.z * src.y.z;
    src.y -= src.z * src.y.z;
    result.x -= result.z * src.x.z;
    src.x -= src.z * src.x.z;
